...
with tab_lex:
    ...
    if query.strip():
        search_key = f"lex_search_{query.strip().lower()}"
        if not st.session_state.get(search_key):
            add_event(st.session_state.user['id'], 'lexicon_search')
        ...
        with st.container(border=True):
            ...
            if st.button(f'Zusammenfassung laden und übernehmen: {hit["title"]}', key=f'add_{hit["title"]}', use_container_width=True):
                try:
                    ...
                    add_event(st.session_state.user['id'], 'wiki_added')
                    st.success(f'„{entry["titel"]}“ wurde für diese Sitzung ergänzt. +20 Punkte')
                    st.rerun()
                except Exception as e:
            ...
            for i, c in enumerate(candidates, start=1):
                with st.container(border=True):
                    ...
                    if st.button('Diesen Eintrag speichern', key=f'save_upload_{i}', use_container_width=True):
                        ...
                        add_event(st.session_state.user['id'], 'upload_entry')
                        st.success('Eintrag gespeichert. Er erscheint im Lexikon. +30 Punkte')
                        st.rerun()
