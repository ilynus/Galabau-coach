...
from .db import init_db, create_user, verify_user, get_or_create_user


def _secret_admin_login(username: str, password: str):
    """Prüft den festen Admin-Login aus Streamlit Secrets."""
    admin_user = str(st.secrets.get("ADMIN_USER", "admin")).strip()
    admin_password = st.secrets.get("ADMIN_PASSWORD", None)

    if admin_password and username == admin_user and password == str(admin_password):
        return get_or_create_user(admin_user, str(admin_password))
...
def require_login():
    ...
    with tab_login:
        ...
        if st.button('Einloggen', type='primary'):
            ...
            # 1. Fester Admin-Login über Streamlit Secrets
            user = _secret_admin_login(username_clean, password)
            if user:
                st.session_state.user = {'id': user['id'], 'username': user['username']}
                st.rerun()

            # 2. Normaler Login über registrierte Nutzer in der Datenbank
